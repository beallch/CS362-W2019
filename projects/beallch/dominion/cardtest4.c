...
int main() {
        ...
        copy.whoseTurn = 0;
	copy.hand[0][0] = great_hall;
        cardEffect(great_hall, 0,0,0, &copy, 0, &bonus);

	printf("Test 1: Effects\n");
        fails = 0;
        fails += asserttrue(copy.numActions == game.numActions + 1);
        fails += asserttrue(copy.handCount[0] == game.handCount[0]);
        fails += asserttrue(copy.playedCards[copy.playedCardCount-1] == great_hall);
        if (fails)
                printf("Failed\n");
        else
                printf("Success\n");
